...
# removable media from the Pop!_OS APT repositories (staging excluded). Write
...
#
# Same Pop!_OS differences as build-popos.sh (CDN repos, system76 kernel
# option, separate workspace/cache/config).
...
function host_help() {
    if [ -z "${1+x}" ]; then
        echo "This script builds a ready-to-flash Pop!_OS raw disk image (.img) for USB sticks, SD cards, CompactFlash, and similar removable media (repos from https://apt.pop-os.org/, staging excluded)."
        echo
    else
        echo "$1"
        echo
    ...
    echo "  --disk-size=32|64|128|<GB>               Disk image size in GB (default 16; minimum 8)"
    ...
    echo "  --generate-config                        Launch config wizard to generate a build-popos-removable.cfg file"
    ...
    echo "                   GRUB, cloud-init, the desktop unless --profile=cli, browsers, the"
    ...
    echo "  Size      --disk-size=32|64|128|<GB> (minimum 8). Fixed layout: ESP 512 MB +"
    ...
    echo "  $0 --advanced --config=build-popos-removable.cfg --no-interactive -    Unattended from a config"
...
function configure_removable_cloud_init() {
    ...
    local seed_host="${TARGET_HOSTNAME:-pop-os}"
    host_priv tee "$IMG_MOUNT_DIR/etc/cloud/cloud.cfg.d/90-uvb-removable.cfg" >/dev/null <<'EOF'
# Written by the Ubuntu Vanilla removable image builder (Pop!_OS variant).
...
function interactive_mode_pick() {
    ...
    echo "    2) Advanced  Adds config file loading (build-popos-removable.cfg / --config),"
...
function interactive_firmware_pick() {
    ...
    while true; do
        ...
        case "${choice,,}" in
            ""|1|u|uefi|efi|uefi-only) export TARGET_FIRMWARE="hybrid";   break ;;
...
function resolve_firmware_choice() {
    if [[ -n "${TARGET_FIRMWARE:-}" ]]; then
        ...
        case "$TARGET_FIRMWARE" in
            ...
            uefi-only)   export TARGET_FIRMWARE="hybrid" ;;
        esac
        return 0
    fi
...
function resolve_workspace_paths() {
    ...
    WORKSPACE_DIR="${ws_parent%/}/workspace-popos-removable"
...
function generate_config_wizard() {
    ...
    local out_path="$SCRIPT_DIR/build-popos-removable.cfg"

    ui_banner "Build Configuration Wizard"
    echo "  This wizard will generate a build-popos-removable.cfg file with your settings."
...
WIZARD_EOF
    ...
    ui_info "Run './build-popos-removable.sh -' to start a build with these settings."
    exit 0
}
...
function host_main() {
    ...
    if [[ "${ADVANCED_MODE:-0}" == "1" ]]; then
        ...
        elif [[ -f "$SCRIPT_DIR/build-popos-removable.cfg" ]]; then
            load_config_file "$SCRIPT_DIR/build-popos-removable.cfg"
        fi
    ...
    ui_banner "Pop!_OS Vanilla Cloud Image Builder"
    ui_kv "Script"     "$0"
    ui_kv "Workspace"  "$WORKSPACE_DIR"
